...
namespace Snoopy.Common.Modes
{
    public class BlowFish
    {
        ...
        public BlowFish()
        {
            ...
            SetupKey(HexToByte("14002291F70839EE"));
        }
        ...
        public string Encrypt_CBC(string pt)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Encrypt_CBC(Encoding.Unicode.GetBytes(pt)));
        }
        ...
        public string Decrypt_CBC(string ct)
        {
            ...
            return Encoding.Unicode.GetString(Decrypt_CBC(HexToByte(ct.Substring(16)))).Replace("\0", "");
        }
        ...
        /// </summary>
        /// <param name="pt">Plaintext to encrypt as Unicode string</param>
        /// <returns>hex value of encrypted data</returns>
        public string Encrypt_ECB(string pt)
        {
            return ByteToHex(Encrypt_ECB(Encoding.Unicode.GetBytes(pt)));
        }
        ...
        /// <returns>Plaintext Unicode string</returns>
        public string Decrypt_ECB(string ct)
        {
            return Encoding.Unicode.GetString(Decrypt_ECB(HexToByte(ct))).Replace("\0", "");
        }
        ...
        public string Encrypt_CTR(string pt)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Crypt_CTR(Encoding.Unicode.GetBytes(pt), 2));
        }
        ...
        {
            IV = HexToByte(ct.Substring(0, 16));
            return Encoding.Unicode.GetString(Crypt_CTR(HexToByte(ct.Substring(16)), 2)).Replace("\0", "");
        }
